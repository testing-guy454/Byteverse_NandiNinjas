...
export const fetchAllPYQs = () => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get("http://localhost:4000api/v1/pyq/all", { withCredentials: true });
...
//   try {
//     await axios.post("http://localhost:4000api/v1/pyq/admin/add", pyqData, { withCredentials: true });
...
export const addPYQ = (pyqData) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(
      "http://localhost:4000/api/v1/pyq/admin/add",
      pyqData,
